                ...
                <ul className="codeboxr-main-menu">
                  {headerMenu.map((item, index) =>
                    item.hassubmenu === 0 ? (
                      ...
                      <li key={index} className="has-submenu menu-item-depth-0">
                        <a href={item.url}>{item.name}</a>
                        <ul className="sub-menu">
                          {item?.innerList?.map((val,subIndex) =>
                            val?.hassubmenu === 0 ? (
                              <li key={subIndex}>
                                <a href={val.url}>{val.name}</a>
                              </li>
                            ) : (
                              <li key={subIndex} className="has-submenu">
                                <a href={val.url}>{val.name}</a>
                                <ul className="sub-menu">
                                  {val?.GallaryList.map((subItem,subSubIndex) => {
                                    return (
                                      <li key={subSubIndex}>
                                        <a href={subItem.url}>{item.name}</a>
                                      </li>
                                    );
                                  })}
                                </ul>
